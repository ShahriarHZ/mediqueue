...
import { toast } from "react-toastify"; // ✅ FIXED: Changed from 'react-hot-toast' to match your layout's ToastContainer
...
export default function MyBookings() {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await axiosSecure.delete(`/bookings/${activeBookingForDelete.id}`);
      if (response.data.success) {
        // ✅ This toast alert will now pop up instantly on your screen!
        toast.success(`Successfully cancelled session with ${activeBookingForDelete.tutor_name}.`);
        
        setBookings((prev) => prev.filter((b) => b.id !== activeBookingForDelete.id));
        setActiveBookingForDelete(null); 
      }
    } catch (err) {
      ...
      {bookings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {bookings.map((booking) => (
            ...
            >
              {/* Image Frame */}
              <div className="w-16 h-16 rounded-xl overflow-hidden bg-base-200 shrink-0 border border-base-200 shadow-sm">
                <img 
                  src={booking.photo || "https://images.unsplash.com/photo-1544717305-2782549b5136?q=80&w=150"} 
                ...
                <div className="flex items-center justify-between gap-4 pt-0.5 w-full">
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-semibold text-base-content/60">
                    ...
                    <span className="flex items-center text-primary font-bold">
                      <FaDollarSign className="text-xs -mr-0.5"/>{booking.price || 0}/hr Paid 
                    </span>
                  </div>
            ...
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <p className="text-sm font-medium text-base-content/60 leading-relaxed">
                ...
                <span className="font-bold text-base-content">
                  {activeBookingForDelete.tutor_name}
                </span>
